...
@section('heading', 'Edit Log Entry')

@section('content')
    <div class="max-w-2xl bg-white rounded-2xl border border-line p-8">
        <form method="POST" action="{{ route('student.logs.update', $log) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="log_date" value="{{ old('log_date', $log->log_date->format('Y-m-d')) }}" required
                ...
                <div>
                    ...
                    <input type="number" step="0.5" name="hours_spent" value="{{ old('hours_spent', $log->hours_spent) }}" required
                           class="w-full rounded-xl border border-line px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="6" required
                          class="w-full rounded-xl border border-line px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent/50 focus:border-accent">{{ old('description', $log->description) }}</textarea>
            </div>
